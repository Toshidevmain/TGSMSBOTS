...
bot.onText(/\/help/, msg => {
  ...
  bot.sendMessage(userId, `📖 *𝗛𝗢𝗪 𝗧𝗢 𝗨𝗦𝗘 𝗧𝗛𝗘 𝗕𝗢𝗧*\n\n/start - Welcome message\n/request - Request access\n/bomb [number] [seconds] - Start bombing (must be approved)\n\n*Example:* /bomb 09123456789 60\n\n⚠️ Don't abuse or spam.`, { parse_mode: 'Markdown' });
});
...
bot.on('callback_query', async query => {
  ...
  if (action === "approve") {
    ...
    bot.once('message', async (response) => {
      ...
      bot.sendMessage(userId, `🎉 𝗔𝗖𝗖𝗘𝗦𝗦 𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗 𝗙𝗢𝗥 ${days}d ${hours}h ${minutes}m. Use /bomb`);
